...
public class EnemyTroop extends CombatTroop {
    ...
    public EnemyTroop(int[][] MonIndex, int MonNumber, EntityManager entitymanager, Handler handler, float x, float y, ID id) {
        ...
        inital = x;
        setMaxhealth(MonIndex[MonNumber][3]);
        setHealth(maxhealth);
    ...
    public void die() {
        for (Entity e : handler.getWorld().getEntityManager().getEntities()) {
            if (e.getID() == ID.PlayerTroop) {
                PlayerTroop playertroop = (PlayerTroop) e;
                for (Entity e1 : handler.getWorld().getEntityManager().getEntities()) {
                    if (e1.getID() == ID.Player) {
                        PlayerShip playerShip = (PlayerShip) e1;
                        playerShip.setHealth(playertroop.health);
                        playerShip.setAtk(playertroop.atk);
                        playerShip.setDef(playertroop.def);
                        break;
                    }
                }

                for (Entity e2 : handler.getWorld().getEntityManager().getEntities()) {
                    if (e2.getID() != ID.Player && e2.getID() != ID.Enemy) {
                        e2.setActive(false);
                    }
                }
            }
